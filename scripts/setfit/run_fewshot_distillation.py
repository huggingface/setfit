...
class RunFewShotDistill:
    def __init__(self, args, mode, trained_teacher_model, x_train_teacher, student_train_ds) -> None:
        ...
        if mode == self.BASELINE_STUDENT:  
            ...
            self.bl_stdnt_distill = baseline_distill(args.baseline_student_model, 
        ...
        for dataset, metric in self.dataset_to_metric.items():
            ...
            for name in fewshot_ds:
                ...
                if (self.mode == self.BASELINE_STUDENT):
                    ...
                    print("Baseline model score: ", metrics[metric] * 100)
    ...
    def train_baseline_student(self, train_data, test_data, num_classes):
        ...
        train_df_student_prob = pd.DataFrame({
        'text': x_train,
        'score': list(y_train_teacher_pred_prob)
            })

        train_raw_student_prob = Dataset.from_pandas(train_df_student_prob)
        
        metric = self.bl_stdnt_distill.standard_model_distillation(train_raw_student_prob, 
                                                          x_test,                                                                
                                                          y_test,
                                                          num_classes                                
                                                          )   
...
def main():
    ...
    fewshot_teacher = RunFewShotDistill(args, 
                                        mode=TEACHER, 
